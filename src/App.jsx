...
function ProductCard({ p, onAdd, onOpenDetail }) {
  ...
  return (
    ...
    >
      ...
      <button onClick={() => onOpenDetail(p)} className="w-full aspect-square flex items-center justify-center" style={{ background: "#1B1C24" }}>
        {cover ? (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.8)" }} onClick={onClose}>
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-lg" style={{ background: C.card, border: `1px solid ${C.border}` }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid md:grid-cols-2 gap-6 px-6 pb-6">
          {/* Galerie */}
          <div>
            <div className="aspect-square rounded-md overflow-hidden flex items-center justify-center relative" style={{ background: "#1B1C24" }}>
              {images.length > 0 ? (
              ...
              <div className="flex gap-2 mt-2">
                {images.map((img, idx) => (
                  <button
                    ...
                    style={{ border: `2px solid ${activeImg === idx ? C.violet : C.border}` }}
                  >
          ...
          <div>
            ...
            <div className="border-t pt-4" style={{ borderColor: C.border }}>
              ...
              {reviewSent ? (
                <p className="text-xs" style={{ color: C.violet }}>Merci pour ton avis ! 🙏</p>
              ) : (
...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Inter, sans-serif" }}>
      {/* Bandeau promo */}
      <div className="text-center py-2 text-xs" style={{ background: C.violet, color: "#fff" }}>
