...
async function fetchPromoCode(code) {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/promo_codes?select=*&code=eq.${encodeURIComponent(code.toUpperCase())}&active=eq.true`,
      { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
    );
    ...
    const promo = rows[0];
    if (promo.expires_at && promo.expires_at < new Date().toISOString().slice(0, 10)) return null;
    return promo;
  } catch (e) {
    console.error("Erreur validation code promo", e);
    return null;
  }
}
...
function ProductCard({ p, onAdd, onOpenDetail }) {
  ...
  return (
    ...
    >
      <button onClick={() => onOpenDetail(p)} className="w-full aspect-square flex items-center justify-center relative" style={{ background: C.imgBg }}>
        {cover ? (
          <img src={cover} alt={p.name} className="w-full h-full object-cover" />
        ) : (
          <Zap size={36} color={C.border} />
        )}
        {p.available_qty != null && p.available_qty > 0 && p.available_qty <= 3 && (
          <span
            className="absolute top-2 left-2 text-[10px] px-2 py-0.5 rounded-full"
            style={{ ...mono, background: C.danger, color: "#fff" }}
          >
            Plus que {p.available_qty} !
          </span>
        )}
      </button>
      <div className="p-4">
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-lg" style={{ background: C.card, border: `1px solid ${C.border}` }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid md:grid-cols-2 gap-6 px-6 pb-6">
          ...
          <div>
            ...
            <div style={{ ...display, color: C.ink }} className="text-2xl mb-2">{fmt(p.price)} DHS</div>
            {p.available_qty != null && p.available_qty > 0 && p.available_qty <= 3 && (
              <p className="text-xs mb-3" style={{ color: C.danger }}>⚡ Plus que {p.available_qty} en stock — commande vite !</p>
            )}
            <button
...
function CheckoutModal({ cart, onClose, onConfirmed }) {
  ...
  const [promoStatus, setPromoStatus] = useState(""); // "" | "checking" | "valid" | "invalid"
  const subtotal = cart.reduce((s, i) => s + i.price * i.qty, 0);
  const promoDiscount = promo ? Math.round((subtotal * promo.discount_percent) / 100) : 0;
  const total = subtotal - promoDiscount;

  const applyPromo = async () => {
    if (!promoInput.trim()) return;
    setPromoStatus("checking");
    const found = await fetchPromoCode(promoInput);
    if (found) {
      setPromo(found);
      ...
      <div className="w-full max-w-md rounded-lg p-6" style={{ background: C.card, border: `1px solid ${C.border}` }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mb-1">
          <input
            ...
            className="flex-1 rounded-md px-3 py-2 text-sm bg-transparent outline-none"
            style={{ border: `1px solid ${C.border}`, color: C.ink }}
          />
          <button onClick={applyPromo} className="px-3 py-2 rounded-md text-xs" style={{ background: C.goldSoft, color: C.ink }}>
            Appliquer
          </button>
        </div>
        {promoStatus === "valid" && <p className="text-xs mb-4" style={{ color: C.gold }}>Code "{promo.code}" appliqué : −{promo.discount_percent}%</p>}
        {promoStatus === "invalid" && <p className="text-xs mb-4" style={{ color: C.danger }}>Code invalide ou expiré.</p>}
        {promoStatus === "" && <div className="mb-4" />}

        {promoDiscount > 0 && (
          <div className="flex justify-between mb-1 text-sm">
            <span style={{ color: C.inkSoft }}>Sous-total</span>
            <span style={{ ...mono, color: C.inkSoft }}>{fmt(subtotal)} DHS</span>
          </div>
        )}
        {promoDiscount > 0 && (
          <div className="flex justify-between mb-1 text-sm">
            <span style={{ color: C.inkSoft }}>Réduction</span>
            <span style={{ ...mono, color: C.gold }}>−{fmt(promoDiscount)} DHS</span>
          </div>
        )}
...
export default function App() {
  ...
  const addToCart = (p) => {
    const cover = (p.images && p.images[0]) || p.image_url;
    const maxQty = p.available_qty != null ? p.available_qty : 99;
    if (maxQty <= 0) return;
    setCart((c) => {
      const existing = c.find((i) => i.id === p.id);
      if (existing) {
        if (existing.qty >= maxQty) return c;
        return c.map((i) => (i.id === p.id ? { ...i, qty: i.qty + 1 } : i));
      }
      return [...c, { id: p.id, name: p.name, price: p.price, image_url: cover, qty: 1, maxQty }];
    });
    setCartOpen(true);
  };
  const changeQty = (id, delta) =>
    setCart((c) => c.map((i) => (i.id === id ? { ...i, qty: Math.max(1, Math.min(i.maxQty || 99, i.qty + delta)) } : i)));
